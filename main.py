...
def page2():
    ...
    try:
        ...
        date_range = st.slider('Select Date Range', min_value=min_date, max_value=max_date,
                               value=(min_date, max_date))  # Getting the input.
        ...
        engage_stats = [
            ...
            str("{0:,.0f}".format(round(
                100 * (df["Engagements"].iloc[-1] - df["Engagements"].iloc[0]) / max(1, df["Engagements"].iloc[0]),
                2))) + "%"
        ]
        ...
        impress_stats = [
            ...
            str("{0:,.0f}".format(round(
                100 * (df["Impressions"].iloc[-1] - df["Impressions"].iloc[0]) / max(1, df["Impressions"].iloc[0]),
                2))) + "%"
        ]
        ...
        output_df["Engagements"] = pd.Series(["{0:,.0f}".format(val) for val in output_df["Engagements"]],
                                             index=output_df.index)
        output_df["Impressions"] = pd.Series(["{0:,.0f}".format(val) for val in output_df["Impressions"]],
                                             index=output_df.index)
        output_df["Percent"] = pd.Series(["{0:,.2f}%".format(val) for val in output_df["Percent"]],
                                         index=output_df.index)
...
def page3():
    ...
    st.markdown(
        "**Note:** this dashboard can be used with only the Engagements & Impressions data if you choose. It is "
        "optional to upload your Shares file, but you won't be able to click through to posts from the chart.")
    ...
    st.markdown(
        "You'll have to request an archive of your data from LinkedIn following the instructions at this [link]("
        "https://www.linkedin.com/help/linkedin/answer/50191/downloading-your-account-data?lang=en). This generally "
        "takes ~24 hours, so go and request that!")
